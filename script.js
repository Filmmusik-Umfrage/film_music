...
function showPrivacyFooterOnStart() {
  const footer = document.querySelector("footer");
  if (!footer) return;

  // Link nur einmal einfügen
  if (!document.getElementById("privacy-link")) {
    footer.innerHTML = `<a href="#" id="privacy-link" style="color:#aaa; text-decoration:underline; cursor:pointer;">Datenschutzhinweis</a>`;
  }
  ...
  if (!document.getElementById("privacy-modal")) {
    ...
    modal.style = "display:none; position:fixed; left:0; top:0; width:100vw; height:100vh; background:rgba(0,0,0,0.7); z-index:9999; align-items:center; justify-content:center;";
    modal.innerHTML = `
...
// Rufe diese Funktion auf, wenn der Startscreen angezeigt wird:
if (startScreen && startScreen.classList.contains("active")) {
  showPrivacyFooterOnStart();
}
